...
def index():
	...
	addDir ('[COLOR white]Pulsa Aqui Para Acceder a los Wizards[/COLOR]'        ,'builds',   icon=ICONBUILDS,   themeit=THEME1)
	addDir ('[COLOR white]Mantenimiento & Herramientas Kodi[/COLOR]'   ,'maint',    icon=ICONMAINT,    themeit=THEME1)
	if wiz.platform() == 'android' or DEVELOPER == 'true': addDir ('Instalar Apps Android' ,'apk', icon=ICONAPK, themeit=THEME1)
	if not ADDONFILE == 'http://': addDir ('[COLOR white]Instala Addons[/COLOR]' ,'addons', icon=ICONADDONS, themeit=THEME1)
	if not YOUTUBEFILE == 'http://' and not YOUTUBETITLE == '': addDir (YOUTUBETITLE ,'youtube', icon=ICONYOUTUBE, themeit=THEME1)
	if HIDECONTACT == 'No': addFile('[COLOR white]Contacto[/COLOR]' ,'contact', icon=ICONCONTACT,  themeit=THEME1)
	if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
	addFile('[COLOR white]Ajustes[/COLOR]'      ,'settings', icon=ICONSETTINGS, themeit=THEME1)
...
def maintMenu(view=None):
	...
	addDir ('[COLOR white][B]Limpia tu Kodi[/B][/COLOR]'       ,'maint', 'clean',  icon=ICONMAINT, themeit=THEME1)
	...
	addDir ('[COLOR white][B]Addons Herramientas[/B][/COLOR]',       'maint', 'addon',  icon=ICONMAINT, themeit=THEME1)
	...
	addDir ('[COLOR white][B]Test de Velocidad & Herramientas de Kodi[/B][/COLOR]'     ,'maint', 'misc',   icon=ICONMAINT, themeit=THEME1)
	...
	addDir ('[COLOR white][B]Buffer / AdvancedSettings by Teco[/B][/COLOR]',       'maint', 'tweaks', icon=ICONMAINT, themeit=THEME1)
...
def removeAddonDataMenu():
	if os.path.exists(ADDOND):
		...
		for folder in sorted(fold, key = lambda x: x):
			...
			replace = {'audio.':'[COLOR white][AUDIO] [/COLOR]', 'metadata.':'[COLOR cyan][METADATA] [/COLOR]', 'module.':'[COLOR white][MODULOS] [/COLOR]', 'plugin.':'[COLOR blue][PLUGIN] [/COLOR]', 'program.':'[COLOR white][PROGRAMAS] [/COLOR]', 'repository.':'[COLOR gold][REPO] [/COLOR]', 'script.':'[COLOR green][SCRIPT] [/COLOR]', 'service.':'[COLOR green][SERVICIOS] [/COLOR]', 'skin.':'[COLOR dodgerblue][SKIN] [/COLOR]', 'video.':'[COLOR white][VIDEOS] [/COLOR]', 'weather.':'[COLOR yellow][TIEMPO] [/COLOR]'}
			for rep in replace:
