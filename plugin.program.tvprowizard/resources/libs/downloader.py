...
class Downloader:
    ...
    def download(self, url, dest):
        ...
        with open(dest, 'wb') as f:
            ...
            if not response:
                error_msg = 'Instalacion Build: Url Zip Invalida!'
                logging.log('Download failed. Invalid ZIP URL: {0}'.format(url), level=xbmc.LOGERROR)
                logging.log_notify(CONFIG.ADDONTITLE,
                                   '[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR2, error_msg))
                return
            else:
