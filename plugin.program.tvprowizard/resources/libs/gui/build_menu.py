...
class BuildMenu:
    ...
    def _list_all(self, match, kodiv=None):
        ...
        for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
            ...
            if not kodiv or kodiv == int(float(kodi)):
                ...
                directory.add_dir('[{0}] {1} (v{2})'.format(float(kodi), name, version), {'mode': 'viewbuild', 'name': name}, description=description, fanart=fanart, icon=custom_icon, menu=menu, themeit=CONFIG.THEME2)
    ...
    def get_listing(self):
        ...
        if response:
            ...
            directory.add_file('Versión de Kodi: {0}'.format(CONFIG.KODIV), icon=CONFIG.ICONBUILDS,
                               themeit=CONFIG.THEME3)
            directory.add_dir('Save Data Menu', {'mode': 'savedata'}, icon=CONFIG.ICONSAVE, themeit=CONFIG.THEME3)
            directory.add_separator()
            directory.add_file('URL inválida para el archivo txt', icon=CONFIG.ICONBUILDS, themeit=CONFIG.THEME3)
        ...
        directory.add_file('Versión de Kodi: {0}'.format(CONFIG.KODIV), icon=CONFIG.ICONBUILDS, themeit=CONFIG.THEME3)
        directory.add_dir('Save Data Menu', {'mode': 'savedata'}, icon=CONFIG.ICONSAVE, themeit=CONFIG.THEME3)
        directory.add_separator()

        if len(match) >= 1:
            directory.add_separator('Builds para KODI 21+', icon=CONFIG.ICONBUILDS, themeit=CONFIG.THEME3)
    ...
    def view_build(self, name):
        ...
        for version, url, gui, kodi, themefile, icon, fanart, preview, adult, info, description in match:
            ...
            if updatecheck:
                build = '{0} [COLOR blue][CURRENT v{1}][/COLOR]'.format(build, CONFIG.BUILDVERSION)
                
            directory.add_file(build, description=description, fanart=fanart, icon=custom_icon, themeit=CONFIG.THEME4)
            directory.add_separator()
            directory.add_dir('Save Data Menu', {'mode': 'savedata'}, icon=CONFIG.ICONSAVE, themeit=CONFIG.THEME3)
            directory.add_file('Información del Build', {'mode': 'buildinfo', 'name': name}, description=description, fanart=fanart,
                               icon=custom_icon, themeit=CONFIG.THEME3)
            ...
            if versioncheck:
                directory.add_file(
                    '[I]Build desarrollado para Kodi v{0} (instalado: v{1})[/I]'.format(str(kodi), str(CONFIG.KODIV)),
                    fanart=fanart, icon=custom_icon, themeit=CONFIG.THEME3)
            ...
            directory.add_file('Instalar', {'mode': 'install', 'action': 'build', 'name': name}, description=description, fanart=fanart,
                               icon=custom_icon, themeit=CONFIG.THEME1)
    ...
    def build_info(self, name):
        ...
        if response:
            if check.check_build(name, 'url'):
                ...
                msg += "[COLOR {0}]Versión de Kodi:[/COLOR] [COLOR {1}]{2}[/COLOR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, kodi)
    ...
    def create_install_menu(self, name):
        ...
        menu_items.append((CONFIG.THEME3.format('Instalación Limpia'), 'RunPlugin(plugin://{0}/?mode=install&name={1}&url=fresh)'.format(CONFIG.ADDON_ID, buildname)))
        menu_items.append((CONFIG.THEME3.format('Instalación Normal'), 'RunPlugin(plugin://{0}/?mode=install&name={1}&url=normal)'.format(CONFIG.ADDON_ID, buildname)))
        ...
        menu_items.append((CONFIG.THEME3.format('Información del Build'), 'RunPlugin(plugin://{0}/?mode=buildinfo&name={1})'.format(CONFIG.ADDON_ID, buildname)))
